...
export async function respondToApproval(rpc: ApprovalRpc, choice: string) {
  ...
  if (!answered || (answered.sessionId && answered.sessionId !== sid)) return
  const response = await rpc('approval.respond', {
    choice, session_id: sid, ...(answered.requestId ? {request_id: answered.requestId} : {})
  })
  const current = getOverlayState().approval
  if (getUiState().sid !== sid || !current ||
      (answered.requestId ? current.requestId !== answered.requestId : current !== answered)) return
  if (!response) return
